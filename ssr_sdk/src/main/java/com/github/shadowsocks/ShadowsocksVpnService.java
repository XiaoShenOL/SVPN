...
public class ShadowsocksVpnService extends BaseService {
    ...
    public void connect() {
        ...
        killProcesses();
        //判断serveraddress
        if (!ToolUtils.isNumeric(profile.host)) {
            String addr = ToolUtils.resolve(profile.host,true);
            if (addr == null) {
               return;
            }
            profile.host = addr;
    ...
    private void startShadowsocksDaemon() {
        ...
        ToolUtils.printToFile(conf, new File(getApplicationInfo().dataDir + "/ss-local-vpn.conf"));
    ...
    private void startDnsTunel() {
        String conf = String.format(Locale.ENGLISH,ConfigUtils.SHADOWSOCKS,
                ...
                ConfigUtils.escapedJson(profile.obfs_param),
                ConfigUtils.escapedJson(profile.protocol_param));
        ToolUtils.printToFile(conf,new File(getApplicationInfo().dataDir+
        "/ss-tunnel-vpn.conf"));
    ...
    private void startDnsDaemon() {
        ...
        ToolUtils.printToFile(conf,new File(getApplicationInfo().dataDir+"/pdnsd-vpn.conf"));
        ArrayList<String> cmd = new ArrayList<>();
    ...
    private void startShadowsocksUDPDaemon() {
        String conf = String.format(Locale.ENGLISH,ConfigUtils.SHADOWSOCKS,
                ...
                ConfigUtils.escapedJson(profile.obfs_param),
                ConfigUtils.escapedJson(profile.protocol_param));
        ToolUtils.printToFile(conf,new File(getApplicationInfo().dataDir+"/ss-local-udp-vpn.conf"));
        ArrayList<String> cmd = new ArrayList<>();
        cmd.add(getApplicationInfo().dataDir+"/ss-local");
        cmd.add("-V");
        cmd.add("-U");
    ...
    private int startvpn() {
        ...
        tun2socksPrcess = new GuardProcess(cmd).start();
        return mFd;
    }
        ...
        if (-1 != fd) {
            ...
            while (tries < 5) {
                try {
					...
					String sock_path = getApplicationInfo().dataDir + "/sock_path";
                    if (com.github.shadowsocks.System.sendfd(fd, sock_path) != -1) {
                        return true;
                    }
