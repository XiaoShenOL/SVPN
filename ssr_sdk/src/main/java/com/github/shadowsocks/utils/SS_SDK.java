...
public class SS_SDK implements SetProfile {
    ...
    public void copyAssets(Context context) {
        ...
        copyAssets(com.github.shadowsocks.System.getABI(), context);
        ...
        Shell.SH.run(cmd);
        cmd.clear();
        editor.putInt(Key.currentVersionCode, this.getVersionCode(context)).apply();
    }

    public void updateAssets(Context context) {
        if (settings.getInt(Key.currentVersionCode, -1) !=  this.getVersionCode(context))
            copyAssets(context);
    }
